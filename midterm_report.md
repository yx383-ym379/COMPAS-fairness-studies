...
General speaking, for individuals from any two groups in the protected feature, e.g. an African-American individual and a Caucasian individual in the protected feature `race`, we expect a fair model to predict them to risky or not with similar probability. Mathematically, suppose we use distance measure function $J(\cdot,\cdot)$, protected feature $D$, and prediction $\hat{y}\in Y=\{\text{will recividate},\text{won't recividate}\}$, the bias is measured by :

$$J(Pr(\hat{y}\in Y|d_1\in D),\enspace Pr(\hat{y}\in Y|d_2\in D))$$

A fair model would have $J(\cdot,\cdot)\leq \epsilon$, where $\epsilon$ represents a small value. 
...
For a fair model, we expect it to have similar performance for any two groups of the protected features, i.e. very small $J(\cdot,\cdot)$ value. 

**Choice of distance measure $J(\cdot,\cdot)$**:
...
for probabilities $p$ and $q$. When we take $\epsilon=0.2$, $J(\cdot,\cdot)\leq \epsilon$ represents the 80% rule. 
...
Mathematically, this can be expressed as $d(x_1,x_2)\leq\epsilon\Rightarrow Pr(\hat{y}|x_1)\approx Pr(\hat{y}|x_2)$, where $d(x_1,x_2)$ is a distance function, and $x_1$,$x_2$ are visible features of two data points. 
